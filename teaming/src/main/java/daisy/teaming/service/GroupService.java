...
public class GroupService {
    ...
    public Result addGroup(Group group, int projectId, HttpServletRequest request)
    {
        ...
        try{
            ...
            groupMapper.addGroupProject(group.getGroupId(), projectId);     //建立小队-比赛表
            groupMapper.addUserGroup(account, groupId);
            result.setSuccess(true);
            result.setMsg("发布成功");
    ...
    //组长审核申请
    public Result updateApplication(int groupId, int applicationId, Application application, HttpServletRequest request)
    {
        Result result=new Result();
        result.setSuccess(false);
        result.setMsg(null);
        ...
        try{
            ...
            User user=userMapper.getUser(account);
            Group group = groupMapper.getGroup(groupId);
            int applyResult = application.getResult();
            String applyAccount = application.getAccount();
            if(account.equals(group.getLeaderAccount())) {
                groupMapper.updateApplication(applyResult,applicationId);       //更新申请状态
                groupMapper.addUserGroup(applyAccount, groupId);     //建立用户-小队表
                result.setSuccess(true);
