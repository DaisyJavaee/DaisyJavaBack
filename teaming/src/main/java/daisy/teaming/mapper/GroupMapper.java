...
public interface GroupMapper {
    ...
    @Insert("insert into `group` values(#{groupId},#{name},#{leaderAccount},#{introduction},#{maxNum})")
...
//  @SelectProvider(type = GroupSqlBuilder.class, method = "buildGetGroupList")
    ...
    @Select("select name from user where account in (select account from userGroup where groupId=#{groupId})")
